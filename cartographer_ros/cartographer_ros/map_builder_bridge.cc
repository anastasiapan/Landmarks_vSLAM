...
std::string MarkerShape(const std::string& landmark_id) {
  std::string delimiter = "_";
  std::string object_id = landmark_id.substr(0, landmark_id.find(delimiter));
  std::string shape;

  if (object_id.compare("Cone") == 0) {
    shape = "file:///home/anastasia/catkin_ws/src/cartographer_ros/cartographer_ros/cartographer_ros/models/Construction_Cone/meshes/construction_cone.dae"; } // SPHERE for a fire extinguisher
  else {
    shape = "file:///home/anastasia/catkin_ws/src/cartographer_ros/cartographer_ros/cartographer_ros/models/Fire_Extinguisher/meshes/extinguisher.dae";} 
  return shape;
}

double MarkerScale(const std::string& landmark_id) {
  std::string delimiter = "_";
  std::string object_id = landmark_id.substr(0, landmark_id.find(delimiter));
  double kLandmarkMarkerScale;

  if (object_id.compare("Cone") == 0) {
    kLandmarkMarkerScale = 6; } // SPHERE for a fire extinguisher
  else {
    kLandmarkMarkerScale = 1; }
  return kLandmarkMarkerScale;
}

constexpr double kLandmarkMarkerScale = 10;

visualization_msgs::Marker CreateLandmarkMarker(int landmark_index,
                                                const Rigid3d& landmark_pose,
                                                const std::string& frame_id,
                                                std::string marker_shape, double kLandmarkMarkerScale) {
  visualization_msgs::Marker marker;
  marker.ns = "Landmarks";
  ...
  marker.mesh_resource = marker_shape;
  marker.header.stamp = ::ros::Time::now();
  marker.header.frame_id = frame_id;
...
visualization_msgs::MarkerArray MapBuilderBridge::GetLandmarkPosesList() {
  ...
  for (const auto& id_to_pose : landmark_poses) {
    landmark_poses_list.markers.push_back(CreateLandmarkMarker(
        ...
        id_to_pose.second, node_options_.map_frame, MarkerShape(id_to_pose.first), MarkerScale(id_to_pose.first)));
  }
